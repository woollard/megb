...
| MEGB-03H.2C.3B.2B.3 — In-process fault-injection, recovery, and resumption conformance | **Executed, pending your acceptance.** No B.2C or B.3 begun; no GCP authentication, `gcloud` invocation, cloud-resource access, Docker command of any kind, or privileged-artifact access. Commits, in order: `2d07d9e` (frozen fault-injection plan, committed before any implementation code) / `7673cfb` (implementation + tests + safe report). Ticket checkpoint committed separately, per instruction. Not pushed, per instruction. **Preflight**: `git fetch origin` confirmed `HEAD == origin/main == 716571b`; `origin/main..HEAD` empty; working tree clean; B.2B.1/B.2B.2 and orchestration-schema v3 confirmed accepted and reachable before any code was touched. | **Frozen plan** (`docs/reference/megb-03h2c3b2b3-fault-injection-plan.md`, sha256 `9bf39c6bae0b43e3ce0807d9ca1bd6f08f892b7356f563c73e75376b7bbd9891`, git blob `b037ed68292ed06fc226453ed07130992783db6b`) enumerates, before any implementation code was written: 5 admission boundaries (A1-A5: before reservation / after reservation-before work creation / after work creation-before audit intent / after audit intent-before publication / after publication-before return), 9 worker boundaries (W1-W9: after delivery-before lease / after lease-before executor / retryable failure / non-retryable failure / after artifact write-before outbox intent / after outbox intent-before commit / after commit-before finalization / after finalization-before ack / after ack-before return), 13 additional fault categories (C1-C13: audit-sink failure, outbox-full, orphan-intent abandonment, queue duplicate/redelivery, ack failure, lease expiry/reassignment, stale/future fencing, cancellation at each phase, retry success/exhaustion, conflicting result/cost, coordinator recreation, reservation-without-work, budget exhaustion/orphan recovery), 10 concurrency/race scenarios (R1-R10: concurrency 1/2/4 under a generic synthetic policy, personal-policy rejection above two, reversed completion order, duplicate deliveries racing, cancellation racing commit, lease expiry racing commit, outbox dispatch racing reconciliation, concurrent reservations at the exact ceiling), and 13 invariants (I1-I13, matching the authorization's own list verbatim) -- frozen before any test result was observed. New safe, self-checksummed, versioned report schema `src/distributed/fault_conformance.py` (`FAULT_CONFORMANCE_REPORT_SCHEMA_VERSION = megb-03h2c3b2b3-fault-conformance-v1`, registered in `docs/reference/version-registry.md`, independent of both `DISTRIBUTED_PROVENANCE_SCHEMA_VERSION` and `DISTRIBUTED_ORCHESTRATION_SCHEMA_VERSION`) mirroring the established `H2C2BQualificationReport` pattern: `ConformanceEntry` (fault_point/invariant/passed/attempt_count, closed enums only, never free text); `FaultConformanceReport` derives `total_attempts`/`passed_count`/`failed_count`/`readiness` entirely from its own entries at construction time -- readiness can never be asserted independently of whether every entry passed. All fault injection is deterministic and in-process only: scripted executor outcomes, direct authoritative-store calls simulating crash windows (e.g. acquiring a lease directly against `AtomicWorkStore` to construct a specific interrupted-attempt window), `threading.Barrier`-synchronized races, and the injected `LogicalClock` -- no wall-clock sleep, process termination, subprocess, Docker, or network access anywhere. "Coordinator recreation" (C11) is proven and explicitly labeled as in-process component recreation (a fresh `Coordinator` instance constructed around the exact same in-memory store/queue/outbox objects a prior, discarded instance used) -- never a durable-process, cross-host, or cloud-recovery claim. **No defects were found in the accepted B.2B.1/B.2B.2 code.** Every initial test failure during this checkpoint's own development traced to this checkpoint's own test setup, not production logic: (1) directly acquiring a lease against the store (bypassing the coordinator's own admission path) leaves the coordinator's own `_lease_expiry` tracking dict empty, so `check_lease_expiry` correctly returns `False` until the test also simulates that same coordinator-local bookkeeping -- an already-documented, intentional consequence of B.2B.2's own "explicit, caller-driven expiry, never an autonomous timer" design, not a new gap; (2) several test-construction bugs (wrong `scientific_work_id` reused across differently-named work items, a worker-count ceiling too low for a budget-only race scenario, `Lease.lease_generation=0` violating that field's own already-accepted positive-int constraint) were fixed in the tests themselves. | 39 new tests across `tests/test_fault_conformance.py` (one per fault point, each recording a `ConformanceEntry` into a shared module-level recorder), `tests/test_fault_conformance_schema.py` (9, schema construction/validation/round-trip/checksum-tampering/deterministic-sort-order), and `tests/test_fault_conformance_boundaries.py` (7, AST-based dependency-direction/leakage checks for the new module, extending without modifying the established B.2B.2 pattern). The committed conformance report (`docs/measurement/megb-03h2c3b2b3-fault-conformance-report.{json,md}`) records 39 entries covering every fault point and all 13 invariants, 123 total attempts (6 race scenarios repeated 15 times each), **readiness `IN_PROCESS_RECOVERY_READY_FOR_B2C`, zero failures**. Full offline suite **1762 passed / 31 deselected** (up from 1707). `mypy src tests` clean (198 files). `pylint src tests` **10.00/10, exit 0**. The concurrency/race-adjacent test set (`test_fault_conformance.py` plus the five existing B.2B.2 race-adjacent files -- 100 tests together) re-run 15 additional times during verification, zero nondeterministic failures. **Deterministic report regeneration proven**: the report-generation test was run twice (separate `pytest` invocations); the resulting JSON and Markdown files were byte-for-byte identical both times (`diff` confirmed no differences). **Version-registry validation**: `FAULT_CONFORMANCE_REPORT_SCHEMA_VERSION` registered as a new, independent schema family; `DISTRIBUTED_ORCHESTRATION_SCHEMA_VERSION`/`DISTRIBUTED_PROVENANCE_SCHEMA_VERSION` confirmed unchanged (`git diff --stat` on `src/distributed/_checksums.py` empty for this checkpoint). | None from the literal authorized scope. Confirmed: no GCP authentication, no `gcloud` invocation, no cloud-resource creation or inspection, no Docker command of any kind, no privileged-artifact access, no cloud/IaC dependency added, B.2C/B.3 not begun, no accepted `src/reference/*` schema modified, no accepted B.2B.1/B.2B.2 code modified (this checkpoint is additive-only: one new `src/distributed/` module, four new test files, one version-registry row, and the frozen plan/report docs -- `git diff --stat` across both commits confirms no existing `src/distributed/` file changed). The fault-injection plan was frozen and committed (`2d07d9e`) before any implementation code was written or modified, per instruction. No frozen fault case was weakened, narrowed, or deleted to obtain readiness. `backup-before-secret-fix` not pushed; unaffected by this checkpoint. Branch `main`; working tree state given in this checkpoint's own report; not pushed, per instruction. |
