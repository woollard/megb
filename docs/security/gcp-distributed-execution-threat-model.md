...
**Corrected after initial review** (documentation-only correction; no
GCP access occurred in producing it): the architecture doc's own
correction separates candidate *generation* from candidate *execution*
into two non-transitively-permissioned planes
(`gcp-distributed-reference-execution.md` §2). This document's trust
boundaries, threat matrix, and residual-risk statements are corrected
below to reflect that split explicitly, including four new threat rows
(26–29) addressing plane-separation failure modes directly.
...
- The scientific integrity of the calibration/experiment record itself —
  ...
  evidence.
- **The non-transitivity of the reference-execution plane's and
  candidate-generation plane's permissions** — i.e., the guarantee that
  no single identity, and no compromise of one plane, can reach the
  other plane's resources (Vertex credentials from the execution side;
  privileged reference material or the Docker host from the generation
  side).
...
Reference-execution plane (trusted, NO Vertex credential anywhere in this plane):
            execution-worker process, worker VM's Docker daemon,
            execution-coordinator process

Candidate-generation plane (trusted, NO privileged-reference/Docker-host
            credential anywhere in this plane):
            generation-coordinator process (holds the Vertex model-
            invoker role)

Cross-plane channel (the ONLY connection between the two planes above):
            immutable, checksummed candidate manifest in Cloud Storage —
            write access: generation plane only; read access: execution
            plane only; no other path exists

Shared, out-of-band (not part of either plane's own runtime identity):
...
External:   Vertex AI Model Garden/MaaS (Google-operated managed service;
            reachable from the candidate-generation plane's own
            coordinator identity only)
```

The candidate-container boundary is unchanged from MEGB-02. This
document's boundaries are: **personal environment vs. company
environment** (must never cross without explicit, reviewed promotion);
**coordinator/worker vs. Google-managed services** (Pub/Sub, Cloud
...
still follow least privilege); and, **corrected by this pass, the
reference-execution plane vs. the candidate-generation plane** — two
disjoint trusted contexts with disjoint IAM role sets, connected only by
a one-directional, read-only (from the execution side), immutable-
manifest channel. A compromise fully within one plane's own trust
boundary — e.g. a worker process bug, not merely a contained candidate
escape — still cannot reach the other plane's resources, because the
IAM roles simply do not exist on the compromised plane's identity.
...
| 26 | Execution-worker compromise reaching Vertex (model-cost amplification) | Vertex quota/billing, company playground cost ceiling | Reference-execution plane ↔ candidate-generation plane | A fully compromised execution worker (per threat #1's own adversarial-candidate-code assumption, escalated to the worker process itself) attempts to call Vertex to run up cost or generate unauthorized candidates | **Structural, not monitored**: the execution-worker service account is never granted the Vertex model-invoker role, under any circumstance (`gcp-distributed-reference-execution.md` §2.3) — there is no credential to steal or forge from that plane, so the attack has no path to exist, independent of any runtime monitoring | N/A as a runtime detection — the control is the *absence* of a credential, verifiable ahead of time via IAM policy review (an audit, not a monitor) | N/A — nothing to recover from if the credential path never existed; if IAM review ever finds the role mistakenly granted, revoke immediately and treat as a P0 IAM-design regression | Requires the non-transitive IAM grant discipline to actually hold at every Terraform apply — an IaC review/test item, not eliminated by architecture alone | H.2C.3B.1 (IAM role design), H.2C.3E (SRE review confirms no grant exists) |
| 27 | Candidate-generation-plane compromise reaching privileged reference material or the Docker host | Canonical solutions, hidden tests, reference oracle, production reference cache, worker Docker daemon | Candidate-generation plane ↔ reference-execution plane | A compromised generation-plane process (e.g. a Vertex response-parsing bug exploited by an adversarial model response) attempts to read privileged reference content or reach a worker's Docker daemon | **Structural, not monitored**: the generation service account is never granted access to privileged reference/oracle/cache storage or any worker-VM/Docker-daemon-reachable role (`gcp-distributed-reference-execution.md` §2.3) | Same as #26 — an IAM policy audit, not a runtime monitor, is the applicable check | Same as #26 — revoke a mistaken grant immediately if IAM review ever finds one | Same as #26 — requires the IaC grant discipline to hold | H.2C.3B.1, H.2C.3E |
| 28 | Cross-plane message content leakage (Pub/Sub) | Privileged reference evidence, unrestricted candidate source, credentials | Reference-execution plane ↔ candidate-generation plane, via Pub/Sub | A work-item or manifest message on either plane's queue is populated with more than an opaque reference plus allowlisted routing metadata — e.g. privileged case content embedded directly in an execution work item, or a raw exception/diagnostic string in either queue | Opaque work references and allowlisted routing metadata only, by schema design (`gcp-distributed-reference-execution.md` §2.3) — the message schema itself has no field for privileged content, unrestricted candidate source, or free-form diagnostics, mirroring this project's own established structurally-excluded-field discipline (`calibration_schema.py`, `H2C2BQualificationReport`) | Message-schema leakage tests (the same allowlist-scan discipline already applied to committed safe reports, extended to message payload schemas) | Reject/quarantine a message that fails schema validation before it is ever processed | Requires the message schema to actually be designed this way at H.2C.3B.1/B.2 — not yet implemented, flagged here so it is not overlooked | H.2C.3B.1 (schema design), H.2C.3B.2 (offline synthetic tests) |
| 29 | Cross-plane privilege escalation via a shared or reused identity | Both planes' own non-transitivity guarantee | Reference-execution plane ↔ candidate-generation plane | An operator, IaC bug, or convenience shortcut grants the same service account (or a role bundling both planes' permissions) to both planes, silently collapsing the separation this correction establishes | Distinct service accounts per plane, enforced in IaC (Terraform module boundaries per `gcp-distributed-reference-execution.md` §4); no `Owner`/`Editor`-style broad role that would implicitly bundle both planes' permissions (already forbidden generally by threat #15) | IAM policy review confirming the two service accounts' role sets are disjoint (a concrete, automatable check: intersect the two role sets, assert empty) | Split the shared identity into two; audit what was accessible during the collapsed period | Requires this specific disjointness check to actually run, not just be implied by "separate service accounts exist" | H.2C.3B.1 (identity design), H.2C.3E (SRE review) |
